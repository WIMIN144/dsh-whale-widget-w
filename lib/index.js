...
var MENU_TIPS = {
  ...
  lock: '峰时锁定：峰时勾选后「常驻」自动切到峰谷显示；峰时（工作日 9:00-12:00 / 14:00-18:00）隐藏 DeepSeek 会话的发送按钮并拦截键盘发送，气泡按轮播间隔交替显示峰谷样式与锁定中视图，谷时自动恢复',
...
function rebuildUsageUnits() {
  ...
  var scopes = usageSource === 'hybrid' ? ['today', 'month'] : ['today']
...
function buildUsageLines() {
  ...
  var reasoning = Number(d.reasoning) || 0
  var hit = (cache + miss) > 0 ? Math.round(cache / (cache + miss) * 100) + '%' : '--'
  var costTotal = (Number(d.costCache) || 0) + (Number(d.costMiss) || 0) + (Number(d.costOut) || 0) + (Number(d.costReasoning) || 0)
  var totalTok = cache + miss + out + reasoning
  var esc = String(unit.model || '未知模型').replace(/[&<>"]/g, function (ch) { return { '&': '&amp;', '<': '&lt;', '>': '&gt;', '"': '&quot;' }[ch] })
  return [
    { t: unit.scope === 'today' ? '今日用量' : '本月用量', s: 'A', c: '' },
    { t: esc, s: 'C', c: '' },
    { t: '<span class="dshwv-utok">' + fmtTok(totalTok) + '<em>tok</em></span><span class="dshwv-ucost">' + fmtCost(costTotal) + '</span>', s: 'U', h: 1, c: '' },
...
function apply(ctx) {
    ...
    function handleSessionEvent(sessionId, event) {
      try {
        ...
        // 输出与思考分开计价（同档位）分桶，展示口径与 DSH 弹窗一致（输出=RESPONSE_TOKEN）
        ...
        const costReasoning = (reasoning / 1e6) * p.out[off]
        agg.tokens += input + cache + output + reasoning
        agg.cost += costCache + costMiss + costOut + costReasoning
        agg.lastTs = Date.now()
    ...
    async function refreshOfficialCorrection() {
      ...
      if (!calibrateOk(od, tokenStats.today) || !calibrateOk(om, tokenStats.month)) {
        return officialCorrection // 官方明显滞后：暂不校准
      }
      officialCorrection = {
        at: Date.now(),
        officialToday: od || {},
        officialMonth: om || {},
        snapToday: JSON.parse(JSON.stringify(tokenStats.today || {})),
        snapMonth: JSON.parse(JSON.stringify(tokenStats.month || {})),
      }
    ...
    function mergeHybrid(officialMap, snapMap, localNow) {
      ...
      const cats = ['cache', 'miss', 'out', 'reasoning', 'costCache', 'costMiss', 'costOut']
      const out = {}
    ...
    disposers.push(ctx.webServer.register({
      ...
      handler: async (req, res) => {
        ...
        if (src !== 'local') {
          ...
          if (corr) {
            today = deepseekOnly(mergeHybrid(corr.officialToday, corr.snapToday, tokenStats.today))
            month = deepseekOnly(mergeHybrid(corr.officialMonth, corr.snapMonth, tokenStats.month))
            source = 'hybrid'
          }
        }
        res.writeHead(200, JSON_HEADERS)
